SKILL_NAME = "Lorenz Attractor"
SKILL_DESCRIPTION = "The Lorenz system integrated with explicit Euler -- dx=sigma*(y-x), dy=x*(rho-z)-y, dz=x*y-beta*z with sigma=10, rho=28, beta=8/3 -- traced for 200,000 steps. Project the 3D trajectory onto xy, xz (the canonical butterfly), or yz and accumulate into a 2D density buffer, log-compress, palette-map. The time-ordered palette gradient lets you read the flow direction around the two lobes. Good for \"lorenz\", \"butterfly\", \"strange attractor\", \"chaos\", or any continuous-dynamical-systems motif."
SKILL_KIND = "creation"
SKILL_OWNER = "library"
SKILL_CREATED_AT = 1779667200.0
SKILL_HIDDEN = False
SKILL_CONTROLS = [
    {"type": "enum", "name": "projection", "label": "Projection",
     "options": [
         {"value": "xz", "label": "XZ (butterfly)"},
...
def run(canvas, projection="xz", **_):
    s = int(canvas.size)
