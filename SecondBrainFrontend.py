...
SETTINGS_DATA = [
    ...
    ("LLM Backend", "llm_backend", "The LLM backend for AI Mode. LM Studio is local; OpenAI is cloud-based and requires an API key. Both are private.", "LM Studio", {"type": "dropdown", "options": ["LM Studio", "OpenAI"]}),
...
class App:
    def __init__(self, page: ft.Page):
        ...
        # File pickers for settings:
        self.settings_file_picker = ft.FilePicker(on_result=self.on_settings_pick_result)
        self.settings_dir_picker = ft.FilePicker(on_result=self.on_settings_pick_result)
        self.active_settings_field = None # To track which field to update
        self.page.overlay.extend([self.settings_file_picker, self.settings_dir_picker])
        # Welcome overlay
        ...
        self.reload_backend_btn = ft.ElevatedButton("More Info", icon=ft.Icons.HELP_OUTLINE, on_click=lambda _: self.page.launch_url("https://github.com/henrydaum/second-brain"))
...
# SETTINGS PAGE ---
        ...
        self.settings_view = ft.Container(
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ft.Text("    Second Brain Settings", size=24, weight="bold"),
                            ft.IconButton(
        ...
        if control_type == "bool":
            ...
            if existing_key:
                ...
                value_control = ft.TextField(
                    ...
                    password=True,
                    hint_text="",
                    helper_text="✓ API key is saved (leave blank to keep current or reset to default to delete)",
                    expand=True
                )
            else:
                # No key saved yet
                value_control = ft.TextField(
                    ...
                    helper_text="Will be encrypted and stored securely in Windows Credential Manager. Resetting to default deletes the key.",
                    expand=True
                )
            control_row_controls.append(value_control)
    ...
    def open_settings(self, e=None):
        ...
        self.settings_view.alignment = ft.alignment.center
        self.page.update()
    ...
    def save_api_key_to_keyring(self, api_key: str):
        ...
        import keyring
        if api_key.strip():
            keyring.set_password("SecondBrain", "OPENAI_API_KEY", api_key)
    ...
    def on_settings_pick_result(self, e: ft.FilePickerResultEvent):
    ...
    def _build_welcome_overlay(self):
        ...
        welcome_content = ft.Container(
            content=ft.Column(
                [
                    ft.Text("Welcome to Second Brain!", size=28, weight="bold", text_align="center"),
                    ft.Column(
                        [
                            ft.Row(
                                [
                            ...
                            ft.Text(
                                "This is the main folder (like 'My Documents') where your files are. Second Brain will scan it so you can search it.",
                                size=12,
                            ),
                            ft.ElevatedButton(
                                "Select Directory",
