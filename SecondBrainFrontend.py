...
class BaseLLM:
    ...
    def _build_image_prompt(prompt: str, valid_file_names: list[str]) -> str:
        """Helper to build the text prompt that lists the attached images."""
...
class LMStudioLLM(BaseLLM):
    ...
    def prepare_chat(self, prompt: str, image_paths: list[str] = None):
...
class App:
    def __init__(self, page: ft.Page):
        ...
        # Input field
        self.user_input = ft.TextField(expand=True, multiline=True, label="Compose your message...", shift_enter=True, on_submit=self.send_message, min_lines=1, max_lines=7, max_length=1200, border_radius=15, focused_border_width=2)
        # Send button
    ...
    def _handle_text_to_image_search(self, msg, attachment_chunks, attachment_context_string, lexical_search_term, similar_images, results_column):
        ...
        if self.config['ai_mode'] and self.config['query_multiplier'] >= 1:
            image_queries.extend(self.llm_generate_queries(msg, "image", attachment_context_string, self.config['query_multiplier']))
        ...
        if image_search_results and self.config['ai_mode'] and self.llm_vision and self.config['llm_filter_results']:
            image_search_results = [r for r in image_search_results if self.llm_evaluate_image_relevance(msg, [r['file_path']], attachment_context_string)]
    ...
    def _handle_text_to_text_search(self, msg, attachment_chunks, attachment_context_string, lexical_search_term, image_search_results, similar_images, results_column):
        """Handles the text-to-text search and AI insights logic."""
        from SecondBrainBackend import hybrid_search
        text_queries = []
        if self.config['ai_mode'] and self.config['query_multiplier'] >= 1:
            text_queries.extend(self.llm_generate_queries(msg, "text", attachment_context_string, self.config['query_multiplier']))
        if attachment_chunks:
        ...
        if text_search_results and self.config['ai_mode'] and self.config['llm_filter_results']:
            text_search_results = [r for r in text_search_results if self.llm_evaluate_text_relevance(msg, r['documents'], attachment_context_string)]
    ...
    def _handle_ai_insights(self, image_search_results, text_search_results, msg, attachment_context_string, image_paths, results_column):
    ...
    def send_message(self, e=None):
        ...
        if msg or (attachment and attachment != "[IMAGE]"):
            ...
            lexical_search_term = f"{msg} {"\n\n".join(attachment_chunks)} {attachment_name} {attachment_folder}"
            # parts of image_search_results and image_paths will be used for llm_synthesize_results
            image_search_results, image_paths = self._handle_text_to_image_search(msg, attachment_chunks, attachment_context_string, lexical_search_term, similar_images, results_column)
            # parts of text_search_results will go to llm_synthesize_results
            text_search_results = self._handle_text_to_text_search(msg, attachment_chunks, attachment_context_string, lexical_search_term, image_search_results, similar_images, results_column)
            # llm_synthesize_results - LLM takes results and summarizes and gives insight on them
            full_response = self._handle_ai_insights(image_search_results, text_search_results, msg, attachment_context_string, image_paths, results_column)
    ...
    def attach_files(self, e=ft.FilePickerResultEvent):
        ...
        try:
            ...
            self.attachment_display.value = f"Attached file: {file_path.name} <size: {self.attachment_size} tokens>"
    ...
    def process_attachment(self, path: Path):
        ...
        if not content:
            return
        # Check attachment size in tokens:
    ...
    def image_presentation(self, image_paths: list[str]):
        ...
        for path in image_paths:
            image_preview = ft.Container(
                content=ft.Image(src=path, width=image_preview_size, height=image_preview_size, fit=ft.ImageFit.CONTAIN), on_click=lambda _, p=path: self.page.launch_url(f"file:///{Path(p)}"), tooltip=f"Open image: {path}")  # Can adjust image size HERE
    ...
    def llm_evaluate_text_relevance(self, msg, chunk, attachment_context_string) -> bool:
        # --- Context Preparation ---
...
(Again: respond only with YES or NO.)"""
    ...
    def llm_evaluate_image_relevance(self, msg, image_path, attachment_context_string) -> bool:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_context_string}\n" if attachment_context_string else ""
        user_request = f"USER'S REQUEST:\n'{msg}'\n" if msg else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
(Again: respond only with YES or NO.)"""
    ...
    def llm_generate_queries(self, msg: str, query_type: str, attachment_context_string, n: int = 5) -> list[str]:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_context_string}\n" if attachment_context_string else ""
        user_request = f"USER'S REQUEST:\n'{msg}'\n" if msg else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
Based on the user's prompt, generate {n} creative search queries that could retrieve relevant {content} to answer the user. These queries will go into a semantic search algorithm to retreive relevant {content} from the user's database. The queries should be broad enough to find a variety of related items. These queries will search a somewhat small and personal database (that is, the user's hard drive). Respond with a plain list with no supporting text or markdown.{reminder_suffix}"""
    ...
    def llm_synthesize_results(self, msg, attachment_context_string, relevant_chunks, image_paths, text_paths, target_column=None):
        # --- Context Preparation ---
