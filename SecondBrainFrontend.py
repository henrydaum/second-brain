...
class App:
    ...
    def send_message(self, e=None):
        ...
        if msg or (attachment and attachment != "[IMAGE]"):
            ...
            if self.config['ai_mode'] and (image_search_results or text_search_results):
                results_column.controls.append(ft.Divider())
                ...
                self.llm_synthesize_results(msg, attachment_chunks_string, [r['documents'] for r in text_search_results], image_paths, target_column=results_column)
    ...
    def stream_llm_response(self, prompt: str, image_paths=None, target_column: ft.Column = None):
...
(Again: respond only with YES or NO.)"""
    ...
    def llm_generate_queries(self, user_query: str, type: str, attachment_chunks, n: int = 5) -> list[str]:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks}\n" if attachment_chunks else ""
        user_request = f"USER'S REQUEST:\n'{user_query}'\n" if user_query else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
        content = "documents" if type == "text" else "images"
        reminder_suffix = " Image search is different from text search, so make sure that the queries are optimized for images." if type == "image" else ""
...
Based on the user's prompt, generate {n} creative search queries that could retrieve relevant {content}. These queries will go into a semantic search algorithm to retreive relevant {content} from the user's database. The queries should be broad enough to find a variety of related items. These queries will search a somewhat small and personal database (that is, the user's hard drive). Respond with a plain list with no supporting text or markdown.{reminder_suffix}"""
    ...
    def llm_synthesize_results(self, user_query, attachment_chunks, relevant_chunks, image_paths, target_column=None):
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks}\n" if attachment_chunks else ""
        user_request = f"USER'S REQUEST:\n'{user_query}'\n" if user_query else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
"""
        threading.Thread(target=self.stream_llm_response, args=(prompt, image_paths, target_column), daemon=True).start()
