...
class LMStudioLLM(BaseLLM):
    ...
    def compress_image(self, path):
        ...
        with Image.open(path) as img:
            ...
            if img.mode != 'RGB':
                img = img.convert('RGB')
            # Resize all images to fit within a max size (maintains aspect ratio)
            img.thumbnail((2048, 2048), Image.Resampling.LANCZOS)
            # Save to a new JPEG file
            base, _ = os.path.splitext(path)
            tmp_path = f"{base}_resized.jpg"
            img.save(tmp_path, format="JPEG", quality=80, optimize=True)
        return tmp_path

    def prepare_chat(self, prompt: str, image_paths: str = None):
        ...
        if image_paths:
            ...
            for path in image_paths:
                ...
                if Path(path).suffix.lower() == ".gif":
                    # print("  [SKIPPED] .gif image cannot be processed by LM Studio model")
                    continue
                compressed_path = self.compress_image(path)
                image_handles.append(lms.prepare_image(compressed_path))
                valid_file_names.append(os.path.basename(path))
            # Build the final prompt so that it includes the image sources. This is best done here since this part filters out the .gif files and doing that twice would be annoying.
            source_info = ""
            for i, name in enumerate(valid_file_names):
                source_info += f"\n<Image {i+1}: {name}>"
            final_prompt = f"{prompt}\n\nThe following images are provided:{source_info}\nEach <Image n> corresponds to the nth attached image. When you refer to an image in your response, refer to it by its file name, rather than its number."
            # Create a Chat object and add the user message with the image handle
            chat = lms.Chat()
            chat.add_user_message(final_prompt, images=image_handles)
            return chat
        else:
            # model.respond and model.respond_stream() accept either a string or a Chat object
            return prompt

    def invoke(self, prompt, temperature=1, image_paths=None):
        chat_input = self.prepare_chat(prompt, image_paths)
        return self.model.respond(chat_input, config={"temperature": temperature}).content
    
    def stream(self, prompt, temperature=1, image_paths=None):
        chat_input = self.prepare_chat(prompt, image_paths)
...
class OpenAILLM(BaseLLM):
    ...
    def encode_image(self, image_path):
        # May want to implement some safeguards for huge images here.
        # If the payload is larger than 50MB, it will crash.
        import base64, io
        from PIL import Image, ImageFile
        ...
        with Image.open(image_path) as img:
            ...
            # Save as JPEG with moderate compression (quality=80 is solid)
            img.save(buffer, format="JPEG", quality=80, optimize=True)
            return base64.b64encode(buffer.getvalue()).decode("utf-8")

    def prepare_messages(self, prompt: str, image_paths: list[str] = None) -> list:
        ...
        for path in image_paths:
            if not os.path.exists(path):
                raise FileNotFoundError(f"Image not found at path: {path}")
            # Replicate LMStudio's behavior of skipping GIFs
            if Path(path).suffix.lower() == ".gif":
                # print("  [SKIPPED] .gif image cannot be processed.")
                continue
        ...
        for i, name in enumerate(valid_file_names):
            source_info += f"\n<Image {i+1}: {name}>"
        # This is the same prompt structure
        final_prompt = f"{prompt}\n\nThe following images are provided:{source_info}\nEach <Image n> corresponds to the nth attached image. When you refer to an image in your response, please refer to it by its file name, rather than its number."
    ...
    def invoke(self, prompt, temperature=1, image_paths=None):
        messages = self.prepare_messages(prompt, image_paths)
        # print(f"[DEBUG] Payload size: {len(str(messages)) / 1e6:.6f} MB")
        response = self.client.responses.create(model=self.model_name, input=messages, temperature=temperature)
        return response.output_text

    def stream(self, prompt, temperature=1, image_paths=None):
        messages = self.prepare_messages(prompt, image_paths)
        # print(f"[DEBUG] Payload size: {len(str(messages)) / 1e6:.6f} MB")
        with self.client.responses.stream(model=self.model_name, input=messages) as stream:  # Lacking temperature functionality.
            for event in stream:
...
class App:
    def __init__(self, page: ft.Page):
        ...
        # Top row
        self.top_row = ft.Row([self.ai_mode_checkbox, self.reload_backend_btn, self.sync_directory_btn, self.open_settings_btn, self.reauthorize_button], alignment="spaceAround")
        # Chat list
    ...
    def log(self, text, avatar=None):
        ...
        if avatar == "user":
            message_row = ft.Row([message_content, self.user_avatar], alignment=ft.MainAxisAlignment.END, wrap=True)
    ...
    def llm_worker(self):
        try:
            ...
            # OPEN AI!
            elif self.config.get("llm_backend") == "Open AI":
                self.llm = OpenAILLM(model_name=self.config['openai_model_name'], api_key=self.config.get('openai_api_key', ""))
                # Check for vision; not as straightforward
                model_name_lower = self.llm.model_name.lower()
                known_vision_keywords = ["vision", "gpt-4o", "gpt-5", "gpt-4.1", "o3", "turbo"]
                self.llm_vision = any(keyword in model_name_lower for keyword in known_vision_keywords)
                if self.llm_vision:
                    self.log(f"Model has vision support.")
                else:
                    self.log(f"Model does not have vision support.")
    ...
    def _handle_image_to_image_search(self, attachment, attachment_path, results_column):
        """Handles the image-to-image search logic."""
        similar_images = None
        if attachment == "[IMAGE]":
            ...
            if similar_images:
                similar_image_paths = [result["file_path"] for result in similar_images]
                image_row = self.image_presentation(similar_image_paths)
                ...
                message_row = ft.Row([self.ai_avatar, ft.Markdown(value="No similar images.", selectable=True, extension_set=ft.MarkdownExtensionSet.GITHUB_WEB)])
    ...
    def _handle_text_to_image_search(self, msg, attachment_chunks, attachment_chunks_string, lexical_search_term, similar_images, results_column):
        """Handles the text-to-image search logic."""
        from SecondBrainBackend import hybrid_search
        image_queries = []
        if self.config['ai_mode'] and self.config['query_multiplier'] >= 1:
            image_queries.extend(self.llm_generate_queries(msg, "image", attachment_chunks_string, self.config['query_multiplier']))
        if attachment_chunks:
            image_queries.extend(attachment_chunks)
        if msg:
            image_queries.append(msg)

        image_search_results = hybrid_search(image_queries, lexical_search_term, self.models, self.collections, self.config, "image")
        
        if image_search_results and self.config['ai_mode'] and self.llm_vision and self.config['llm_filter_results']:
            image_search_results = [r for r in image_search_results if self.llm_evaluate_image_relevance(msg, [r['file_path']], attachment_chunks_string)]
        ...
        self.page.update()
        return image_search_results, image_paths

    def _handle_text_to_text_search(self, msg, attachment_chunks, attachment_chunks_string, lexical_search_term, image_search_results, similar_images, results_column):
        """Handles the text-to-text search and AI insights logic."""
        from SecondBrainBackend import hybrid_search
        text_queries = []
        if self.config['ai_mode'] and self.config['query_multiplier'] >= 1:
            text_queries.extend(self.llm_generate_queries(msg, "text", attachment_chunks_string, self.config['query_multiplier']))
        if attachment_chunks:
            text_queries.extend(attachment_chunks)
        if msg:
            text_queries.append(msg)

        prefixed_text_queries = [self.config['text_search_prefix'] + q for q in text_queries]
        text_search_results = hybrid_search(prefixed_text_queries, lexical_search_term, self.models, self.collections, self.config, "text")
        
        if text_search_results and self.config['ai_mode'] and self.config['llm_filter_results']:
            text_search_results = [r for r in text_search_results if self.llm_evaluate_text_relevance(msg, r['documents'], attachment_chunks_string)]
    ...
    def _handle_ai_insights(self, image_search_results, text_search_results, msg, attachment_chunks_string, image_paths, results_column):
        if self.config['ai_mode'] and ((image_search_results and self.llm_vision) or text_search_results):
                results_column.controls.append(ft.Divider())
                ...
                # Ask LLM to synthesize output (hopefully it does a good job) and pass info for saving
                self.llm_synthesize_results(
                    msg=msg, 
                    attachment_chunks_string=attachment_chunks_string, 
                    relevant_chunks=[r['documents'] for r in text_search_results], 
                    image_paths=image_paths, 
                    text_paths=[r['file_path'] for r in text_search_results],
                    target_column=results_column)
    
    def send_message(self, e=None):
        """Handles sending a message, processing attachments, and displaying search results."""
        ...
        # --- Search Logic ---
        similar_images = self._handle_image_to_image_search(attachment, attachment_path, results_column)

        if msg or (attachment and attachment != "[IMAGE]"):
            attachment_chunks = []
            attachment_chunks_string = ""
            if attachment and attachment != "[IMAGE]":
                n_chunks = self.config.get('n_attachment_chunks')
                if n_chunks:
                    attachment_chunks = self.do_attachment_RAG(attachment, [msg], n_chunks=n_chunks)
                    attachment_chunks_string = "\n\n".join(attachment_chunks)
            # Assemble search parts
            attachment_name = attachment_path.name if attachment_path else ""
            attachment_folder = attachment_path.parent.name if attachment_path else ""
            lexical_search_term = f"{msg} {attachment_chunks_string} {attachment_name} {attachment_folder}"
            # parts of image_search_results and image_paths will be used for llm_synthesize_results
            image_search_results, image_paths = self._handle_text_to_image_search(msg, attachment_chunks, attachment_chunks_string, lexical_search_term, similar_images, results_column)
            # parts of text_search_results will go to llm_synthesize_results
            text_search_results = self._handle_text_to_text_search(msg, attachment_chunks, attachment_chunks_string, lexical_search_term, image_search_results, similar_images, results_column)
            # llm_synthesize_results - LLM takes results and summarizes and gives insight on them
            full_response = self._handle_ai_insights(image_search_results, text_search_results, msg, attachment_chunks_string, image_paths, results_column)
    ...
    def stream_llm_response(self, prompt: str, msg: str, image_paths=None, text_paths=None, target_column: ft.Column = None):
        ...
        try:
            ...
            for chunk in self.llm.stream(prompt, 0.6, image_paths):
                if not chunk:
                    continue
                full_response += chunk
        ...
        for r in text_results:            
            ...
            tile = ft.ExpansionTile(title=ft.Text(Path(r['file_path']).stem),
                    ...
                    [ft.ListTile(title=ft.TextButton(text=r['file_path'], on_click=lambda e, p=r['file_path']: self.page.launch_url(f"file:///{Path(p)}")), 
                                subtitle=ft.Text(cleaned_chunk + f" | Result type: {r['result_type']}", selectable=True))])
            # Every result gets a tile
            results_widgets.append(tile)
        results = ft.Column(controls=results_widgets, spacing=0)
        return results

    def llm_evaluate_text_relevance(self, msg, chunk, attachment_chunks_string) -> bool:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks_string}\n" if attachment_chunks_string else ""
        user_request = f"USER'S REQUEST:\n'{msg}'\n" if msg else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
(Again: respond only with YES or NO.)"""
        ...
        response = self.llm.invoke(prompt, 0.01).strip().lower()  # Low temp for objectivity
    ...
    def llm_evaluate_image_relevance(self, msg, image_path, attachment_chunks_string) -> bool:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks_string}\n" if attachment_chunks_string else ""
        user_request = f"USER'S REQUEST:\n'{msg}'\n" if msg else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
(Again: respond only with YES or NO.)"""
        ...
        response = self.llm.invoke(prompt=prompt, temperature=0.01, image_paths=image_path).strip().lower()
    ...
    def llm_generate_queries(self, msg: str, query_type: str, attachment_chunks_string, n: int = 5) -> list[str]:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks_string}\n" if attachment_chunks_string else ""
        user_request = f"USER'S REQUEST:\n'{msg}'\n" if msg else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
Based on the user's prompt, generate {n} creative search queries that could retrieve relevant {content} to answer the user. These queries will go into a semantic search algorithm to retreive relevant {content} from the user's database. The queries should be broad enough to find a variety of related items. These queries will search a somewhat small and personal database (that is, the user's hard drive). Respond with a plain list with no supporting text or markdown.{reminder_suffix}"""
        ...
        # print(f"Generated {type} queries: {cleaned_text_queries}")
        return cleaned_text_queries

    def llm_synthesize_results(self, msg, attachment_chunks_string, relevant_chunks, image_paths, text_paths, target_column=None):
        # --- Context Preparation ---
        from datetime import datetime
        date_time = datetime.now().strftime("%#I:%M %p, %d %B %Y")
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks_string}\n" if attachment_chunks_string else ""
        user_request = f"USER'S REQUEST:\n'{msg}'" if msg else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
        ...
        # --- Refined Prompt ---
        prompt = f"""{self.config['system_prompt']} It is {date_time}.
...
- If there are images, make sure to consider them for your response."""
        threading.Thread(
            ...
            args=(prompt, msg, image_paths, text_paths, target_column),
            daemon=True).start()

    def do_attachment_RAG(self, attachment, queries, n_chunks=3):
        """Attachments are often too large to fit into the LLM's context window, so this function tries to pick the top n most relevant chunks of the attachment based on the queries."""
        import numpy as np
        # Split into chunks
