...
class LMStudioLLM(BaseLLM):
    ...
    def prepare_chat(self, prompt: str, image_paths: list[str] = None, attached_image_path: str = None):
    ...
    def invoke(self, prompt, temperature=1, image_paths=None, attached_image_path=None):
    ...
    def stream(self, prompt, temperature=1, image_paths=None, attached_image_path=None):
        chat_input, temp_files = self.prepare_chat(prompt, image_paths, attached_image_path)
        try:
...
class App:
    ...
    def log(self, text, avatar=None):
        ...
        if avatar == "user" or avatar == "attachment":
            ...
            message_row = ft.Row([message_container, avatar], alignment=ft.MainAxisAlignment.END)
            ...
            message_row = ft.Row([self.ai_avatar, message_container])
            self.chat_list.controls.append(message_row)
        else:
            # If no avatar is specified, message is sent with no avatar, just a boring grey log statement.
            message_container = ft.Container(content=message_content, expand=True, alignment=ft.alignment.center_left)
    ...
    def _handle_image_search(self, msg, attachment_chunks, attachment_context_string, lexical_search_term, image, attached_image_path, results_column):
        ...
        if self.config['ai_mode'] and self.config['query_multiplier'] >= 1:
            image_queries.extend(self.llm_generate_queries(msg, "image", attachment_context_string, self.config['query_multiplier'], attached_image_path))
        if attachment_chunks:
            image_queries.extend(attachment_chunks)
        if msg:
            image_queries.append(msg)
        if image:  # For image-to-image searches
            image_queries.append(image)
        # Perform the search
        image_search_results = hybrid_search(image_queries, lexical_search_term, self.models, self.collections, self.config, "image")
        # Filter out bad results with LLM
        if image_search_results and self.config['ai_mode'] and self.llm_vision and self.config['llm_filter_results']:
            image_search_results = [r for r in image_search_results if self.llm_evaluate_image_relevance(msg, [r['file_path']], attachment_context_string, attached_image_path)]
        # Display results
        image_paths = []
        if image_search_results:
            image_paths = [result['file_path'] for result in image_search_results]
            image_row = self.image_presentation(image_paths)
            if image and not (msg or attachment_chunks):
    ...
    def _handle_text_search(self, msg, attachment_chunks, attachment_context_string, lexical_search_term, image_search_results, attached_image_path, results_column):
        """Handles the text-to-text and image-to-text search logic."""
        from SecondBrainBackend import hybrid_search
        text_queries = []
        if self.config['ai_mode'] and self.config['query_multiplier'] >= 1:
            text_queries.extend(self.llm_generate_queries(msg, "text", attachment_context_string, self.config['query_multiplier'], attached_image_path))
        if attachment_chunks:
        ...
        if not text_queries:  # To enable image to text searches in event of no text input
            text_queries.append(lexical_search_term)
        ...
        if text_search_results and self.config['ai_mode'] and self.config['llm_filter_results']:
            text_search_results = [r for r in text_search_results if self.llm_evaluate_text_relevance(msg, r['documents'], attachment_context_string, attached_image_path)]
        # Display results
    ...
    def _handle_ai_insights(self, image_search_results, text_search_results, msg, attachment_context_string, image_paths, attached_image_path, results_column):
        if self.config['ai_mode'] and ((image_search_results and self.llm_vision) or text_search_results):
            ...
            self.llm_synthesize_results(
                msg=msg, 
                attachment_context_string=attachment_context_string, 
    ...
    def send_message(self, e=None):
        ...
        # --- Attachment Logic ---
        attachment_chunks = []
        attachment_context_string = ""  # For LLM context; given as part of prompt
        attached_image_description = ""
        attached_image = None
        attached_image_path = ""  # For LLM context; image is given as input to vision models
        if attachment:
            # For text attachments:
            if attachment != "[IMAGE]":
                # If larger than maximum, use extracted chunks as context, else, use entire attachment
                attachment_chunks = self.do_attachment_RAG(attachment, [msg])
                if attachment_size > self.config['max_attachment_size']:
                    attachment_context_string = "\n\n".join(attachment_chunks)
                else:
                    attachment_context_string = attachment
            # For image attachments:
            elif attachment == "[IMAGE]":
                attached_image_path = str(attachment_path)
                ...
                try:
                    with Image.open(str(attachment_path)).convert("RGB") as img:
                        attached_image = img
                except Exception as e:
                    print(f" [Warning] Failed to load image {str(attachment_path)}: {e}")
                # Find description of image for search purposes, here with LLM if possible
                if self.config['ai_mode'] and self.llm_vision:
                    attached_image_description = self.llm.invoke(f"Provide a concise description of the content of the attached image for the purpose of searching a personal knowledge base. Do not include the file name in your description.", temperature=0.5, image_paths=[str(attachment_path)], attached_image_path=attached_image_path)
                # If no LLM with vision support, find labels with image embedding
                else:
                    # Embed the image to find labels
                    image_embedding = self.models['image'].encode(attached_image, convert_to_numpy=True, batch_size=self.config['batch_size'], normalize_embeddings=True)
                    ...
                    attached_image_description = ", ".join(labels)
        ...
        lexical_search_term = f"{msg} {attached_image_description} {attachment_context_string} {attachment_name} {attachment_folder}".strip()
        # More context. Doing this part after so that the lexical search term isn't polluted with irrelevant words:
        if attachment:
            if attachment != "[IMAGE]":
                attachment_context_string += f"\nAttachment name: {attachment_path.name}"  # Extra context for LLM
            elif attachment == "[IMAGE]":
                attachment_context_string = f"The user has attached an image: {attachment_path.name}"  # This copies the formatting found in the LLM class for the image prompt. It essentially lets the LLM know which image is the attachment.
        # parts of image_search_results and image_paths will be used for llm_synthesize_results
        image_search_results, image_paths = self._handle_image_search(msg, attachment_chunks, attachment_context_string, lexical_search_term, attached_image, attached_image_path, results_column)
        # parts of text_search_results will be used for llm_synthesize_results
        text_search_results = self._handle_text_search(msg, attachment_chunks, attachment_context_string, lexical_search_term, image_search_results, attached_image_path, results_column)
        # llm_synthesize_results - LLM takes results and summarizes and gives insight on them
        self._handle_ai_insights(image_search_results, text_search_results, msg, attachment_context_string, image_paths, attached_image_path, results_column)
    ...
    def stream_llm_response(self, prompt: str, msg: str, image_paths=None, text_paths=None, attached_image_path=None, target_column: ft.Column = None):
        ...
        if attached_image_path:
            image_paths.append(attached_image_path)  # Ensure attached image is last in the list.

        full_response = ""
        try:
            # [STREAMING] The streaming logic itself, using the "yield" thing
            for chunk in self.llm.stream(prompt, 0.6, image_paths, attached_image_path):
                if not chunk:
                    continue
                full_response += chunk
    ...
    def llm_evaluate_text_relevance(self, msg, chunk, attachment_context_string, attached_image_path) -> bool:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_context_string}\n" if attachment_context_string else ""
...
(Again: respond only with YES or NO.)"""
        ...
        image_paths_proxy = [attached_image_path] if attached_image_path else None
        response = self.llm.invoke(prompt=prompt, temperature=0.01, image_paths=image_paths_proxy, attached_image_path=attached_image_path).strip().lower()  # Low temp for objectivity
    ...
    def llm_evaluate_image_relevance(self, msg, image_paths, attachment_context_string, attached_image_path) -> bool:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_context_string}\n" if attachment_context_string else ""
        user_request = f"USER'S REQUEST:\n'{msg}'\n" if msg else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
(Again: respond only with YES or NO.)"""
        ...
        if attached_image_path:
            image_paths_proxy = image_paths.append(attached_image_path)  # Attached image goes last.
        else:
            image_paths_proxy = image_paths
        response = self.llm.invoke(prompt=prompt, temperature=0.01, image_paths=image_paths_proxy, attached_image_path=attached_image_path).strip().lower()
    ...
    def llm_generate_queries(self, msg: str, query_type: str, attachment_context_string, n: int = 5, attached_image_path=None) -> list[str]:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_context_string}\n" if attachment_context_string else ""
        user_request = f"USER'S REQUEST:\n'{msg}'\n" if msg else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
Based on the user's prompt, generate {n} creative search queries that could retrieve relevant {content} to answer the user. These queries will go into a semantic search algorithm to retreive relevant {content} from the user's database. The queries should be broad enough to find a variety of related items. These queries will search a somewhat small and personal database (that is, the user's hard drive). Respond with a plain list with no supporting text or markdown.{reminder_suffix}"""
        image_paths_proxy = [attached_image_path] if attached_image_path else None
        response = self.llm.invoke(prompt=prompt, temperature=0.7, image_paths=image_paths_proxy, attached_image_path=attached_image_path).strip()
    ...
    def llm_synthesize_results(self, msg, attachment_context_string, relevant_chunks, image_paths, text_paths, attached_image_path, target_column=None):
        # --- Context Preparation ---
...
- If there are images, make sure to consider them for your response."""
        threading.Thread(
            ...
            args=(prompt, msg, image_paths, text_paths, attached_image_path, target_column),
            daemon=True).start()
