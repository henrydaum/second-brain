...
class App:
    ...
    def llm_worker(self):
        try:
            ...
            if self.config.get("llm_backend") == "LM Studio":
                ...
                self.log("Language model (Gemini) successfully loaded")
            else:
    ...
    def _handle_text_to_image_search(self, msg, attachment_chunks, attachment_chunks_string, lexical_search_term, similar_images, results_column):
        ...
        if image_search_results and self.config['ai_mode'] and self.llm_vision:
    ...
    def send_message(self, e=None):
        ...
        if msg or (attachment and attachment != "[IMAGE]"):
            ...
            # image_search_results and image_paths will be used for llm_synthesize_results
            ...
            # text_search_results will go to llm_synthesize_results
            ...
            if self.config['ai_mode'] and ((image_search_results and self.llm_vision) or text_search_results):
                ...
                self.llm_synthesize_results(
                    user_query=msg, 
                    attachment_chunks=attachment_chunks_string, 
    ...
    def stream_llm_response(self, prompt: str, user_query: str, image_paths=None, text_paths=None, target_column: ft.Column = None):
        ...
        self.save_button = ft.IconButton(
            ...
            on_click=lambda _: self.save_insight(
        ...
        try:
            ...
            if full_response.strip(): # Only show if there is content to save
                 ...
                 target_column.controls.append(ft.Row([self.save_button], alignment=ft.MainAxisAlignment.CENTER))
            self.page.update()

    def save_insight(self, insight_text, original_query, image_paths, text_paths):
        ...
        try:
            ...
            self.save_button.icon = ft.Icons.BOOKMARK_ADDED_ROUNDED # Change icon to show it's saved
            self.page.update()
    ...
    def llm_evaluate_text_relevance(self, user_query, chunk, attachment_chunks) -> bool:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks}\n" if attachment_chunks else ""
        user_request = f"USER'S REQUEST:\n'{user_query}'\n" if user_query else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
(Again: respond only with YES or NO.)"""
    ...
    def llm_evaluate_image_relevance(self, user_query, image_path, attachment_chunks) -> bool:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks}\n" if attachment_chunks else ""
        user_request = f"USER'S REQUEST:\n'{user_query}'\n" if user_query else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
(Again: respond only with YES or NO.)"""
    ...
    def llm_generate_queries(self, user_query: str, query_type: str, attachment_chunks, n: int = 5) -> list[str]:
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks}\n" if attachment_chunks else ""
        user_request = f"USER'S REQUEST:\n'{user_query}'\n" if user_query else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
{user_request}
{attachment_context}
Based on the user's prompt, generate {n} creative search queries that could retrieve relevant {content}. These queries will go into a semantic search algorithm to retreive relevant {content} from the user's database. The queries should be broad enough to find a variety of related items. These queries will search a somewhat small and personal database (that is, the user's hard drive). Respond with a plain list with no supporting text or markdown.{reminder_suffix}"""
    ...
    def llm_synthesize_results(self, user_query, attachment_chunks, relevant_chunks, image_paths, text_paths, target_column=None):
        # --- Context Preparation ---
        attachment_context = f"CONTEXT FROM ATTACHMENT:\n{attachment_chunks}\n" if attachment_chunks else ""
        user_request = f"USER'S REQUEST:\n'{user_query}'\n" if user_query else "USER'S REQUEST: The user did not provide a specific prompt; focus on their attachment.\n"
...
- Cite every piece of information you use from the search results with its source, like this: (source_name).
...
"""
        threading.Thread(
            ...
            args=(prompt, user_query, image_paths, text_paths, target_column),
            daemon=True).start()
