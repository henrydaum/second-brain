...
class TelegramFrontend(BaseFrontend):
    ...
    def __init__(self, shutdown_event: threading.Event | None = None, services: dict | None = None):
        ...
        self._plan_messages: dict[str, tuple[int, int]] = {}
    ...
    def render_plan_status(self, session_key: str, payload: dict) -> None:
        """Keep a pinned Telegram plan-progress message in sync with plan events."""
        chat_id = self._chat_id(session_key)
        if not chat_id:
            return
        plan = payload.get("plan") or {}
        key = f"{session_key}:plan:{plan.get('id') or 'active'}"
        self._send(self._upsert_pinned_plan_message(chat_id, key, self._plan_status_html(payload)))
    ...
    async def _send_tool_started(self, chat_id: int, key: str, name: str, text: str):
        ...
        if key in self._tool_messages:
            return await self._progress_tool_message(chat_id, key, name, text)
    ...
    async def _upsert_pinned_plan_message(self, chat_id: int, key: str, text: str):
        """Create or edit the pinned plan-progress message."""
        entry = self._plan_messages.get(key)
        if entry:
            try:
                await self.app.bot.edit_message_text(text, chat_id=entry[0], message_id=entry[1], parse_mode="HTML")
                return
            except Exception:
                self._plan_messages.pop(key, None)
        sent = await self.app.bot.send_message(chat_id, text, parse_mode="HTML", disable_notification=True)
        try:
            await self.app.bot.pin_chat_message(chat_id, sent.message_id, disable_notification=True)
            self._plan_messages[key] = (chat_id, sent.message_id)
        except Exception as e:
            logger.info(f"Could not pin Telegram plan progress message: {e}")

    def _plan_status_html(self, payload: dict) -> str:
        """Render plan progress as compact Telegram HTML."""
        plan, step = payload.get("plan") or {}, payload.get("step") or {}
        steps = plan.get("steps") or []
        total = len(steps) or "?"
        title = html.escape(str(plan.get("title") or "Plan"))
        status = payload.get("status")
        if status == "finished":
            headline = "Plan complete"
        elif status == "failed":
            headline = f"Plan failed on step {step.get('index', '?')}/{total}"
        elif status == "completed":
            headline = f"Completed step {step.get('index', '?')}/{total}"
        else:
            headline = f"Running step {step.get('index', '?')}/{total}"
        lines = [f"<b>{html.escape(headline)}</b>", f"<code>{title}</code>"]
        if step:
            lines.append(html.escape(str(step.get("title") or "")))
        if payload.get("error"):
            lines.append(html.escape(str(payload["error"])))
