"""read_skill_guide: skill authoring template + art_kit introspection.

Bare call →  the contents of templates/skill_template.py plus a tagline
...
The single source of truth for art_kit is plugins/skills/helpers/art_kit.py.
...
from plugins.skills.helpers import art_kit as _art_kit


_TEMPLATE_PATH = Path(__file__).resolve().parents[2] / "templates" / "skill_template.py"
...
def _build_index() -> str:
    ...
    lines = [
        ...
        "(no import needed). Call `read_skill_guide(methods=[\"<name>\", ...])` "
...
class ReadSkillGuide(BaseTool):
    ...
    description = (
        "Return the canvas-skill authoring template (palette discipline, "
        "skill kinds, control descriptors, composition rules, determinism "
        ...
        "skill; call with `methods` whenever you need the exact shape of a "
        ...
        "properties": {
            "methods": {
                ...
                "description": (
                    ...
                    "returns the skill template followed by the art_kit "
                    "tagline index."
                ),
            }
        },
    ...
    def run(self, context, methods=None, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=False, error=f"could not read skill template: {e}")
        text = f"{template}\n\n---\n\n{_build_index()}\n"
