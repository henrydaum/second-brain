"""Hybrid (embedding + BM25) search over canvas skills."""
...
class SearchSkills(BaseTool):
    name = "search_skills"
    description = "Search stored canvas skills by hybrid keyword + semantic ranking over skill name and description."
    max_calls = 6
    ...
    def run(self, context, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult.failed("skill index is not ready yet; let embed_skills and index_skills run first")
...
def search_skills_semantic(db, embedder, query: str, *, limit: int = 5, built_in_only: bool = False, config: dict | None = None) -> list[dict]:
    """Hybrid skill retrieval: BM25 keyword + embedding cosine fused via RRF,
...
def _bm25_candidates(db, query: str, k: int, built_in_only: bool) -> list[tuple[str, int, dict]]:
    ...
    if not match:
        return []
    sql = """
        SELECT slug, name, description, kind, path, bm25(skill_fts) AS rank
        FROM skill_fts
        WHERE skill_fts MATCH ? AND hidden = 0
        ORDER BY rank
        LIMIT ?
    """
    with db.lock:
        try:
            ...
            # Either the FTS5 table hasn't been created yet (index_skills
