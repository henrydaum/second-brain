"""Warm disposable subprocess pool for canvas skill execution."""
...
logger = logging.getLogger("SkillWorkerPool")
READY_TIMEOUT_S = 8.0


class SkillWorkerBusy(RuntimeError):
    """Raised when all warm skill workers are occupied too long."""


class SkillWorkerJobTimeout(RuntimeError):
    """Raised when a worker job exceeds the per-skill timeout."""
...
class SkillWorkerPoolService(BaseService):
    model_name = "skill_worker_pool"
    shared = True
    config_settings = [
        ("Skill Worker Pool Enabled", "skill_worker_pool_enabled", "Use prewarmed disposable subprocesses for canvas skills.", True, {"type": "bool"}),
        ("Skill Worker Min Idle", "skill_worker_min_idle", "Warm sandbox workers kept ready.", 1, {"type": "integer"}),
        ("Skill Worker Max Workers", "skill_worker_max_workers", "Maximum warm/active skill sandbox processes.", 2, {"type": "integer"}),
        ("Skill Worker Queue Timeout (s)", "skill_worker_queue_timeout_s", "Seconds a render waits for a skill worker before failing busy.", 10, {"type": "integer"}),
    ]

    def __init__(self, config: dict):
        super().__init__()
        self.enabled = bool(config.get("skill_worker_pool_enabled", True))
        self.min_idle = max(0, int(config.get("skill_worker_min_idle", 1)))
        self.max_workers = max(1, int(config.get("skill_worker_max_workers", 2)))
        self.min_idle = min(self.min_idle, self.max_workers - 1) if self.max_workers > 1 else 0
        self.queue_timeout_s = max(0.1, float(config.get("skill_worker_queue_timeout_s", 10)))
    ...
    def run_job(self, *, job_path: str, timeout_s: float, memory_mb: int) -> SkillWorkerResult:
        if not self.enabled:
            raise SkillWorkerBusy("skill worker pool is disabled")
        if not self.loaded:
            self.load()
        ...
        if not self._active_sem.acquire(timeout=sem_timeout):
            raise SkillWorkerBusy("renderer busy; retry shortly")
        proc = None
        try:
            with self._lock:
                self._active += 1
    ...
    def _take_idle(self, deadline: float):
        ...
        try:
            ...
            raise SkillWorkerBusy("renderer busy; no warm skill worker became ready")
    ...
    def _ensure_idle(self):
        ...
        with self._lock:
            while (
                ...
                self._warming += 1
                threading.Thread(target=self._warm_one, daemon=True, name="skill-worker-warm").start()
    ...
    def _warm_one(self):
        ...
        try:
            ...
            logger.debug("skill worker warm failed: %s", e)
            if proc is not None:
                self._kill(proc)
        finally:
            with self._lock:
    ...
    def _run_checked_out(self, proc, job_path: str, timeout_s: float, memory_mb: int) -> SkillWorkerResult:
        ...
        try:
            try:
                ...
                self._kill(proc)
                raise SkillWorkerJobTimeout(f"exceeded {timeout_s:.0f}s timeout")
        finally:
            stop.set()
            if wd is not None:
                wd.join(timeout=1.0)
        return SkillWorkerResult(proc.returncode, stdout or b"", stderr or b"", mem["killed"], mem["peak"])
        ...
        def run():
            ...
            while not stop.wait(0.2):
                ...
                if rss > cap:
                    ...
                    self._kill(proc)
                    return
        t = threading.Thread(target=run, daemon=True, name=f"skill-worker-mem-{proc.pid}")
        t.start()
        return t
    ...
    def _worker_context(self):
        ...
        entry = root / "plugins" / "skills" / "helpers" / "skill_sandbox_entry.py"
        env = os.environ.copy()
...
def build_services(config: dict) -> dict:
    return {"skill_worker_pool": SkillWorkerPoolService(config or {})}
