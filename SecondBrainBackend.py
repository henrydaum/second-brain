...
def process_text_file(file_path: pathlib.Path, drive_service, text_splitter, models, is_multimodal, collections, batch_size, log_callback):
    ...
    if not handler:
        _log(f"⦸ Unsupported file type: {file_path.name}", log_callback)
        return
    ...
    if not chunks:
        _log(f"⦸ File is gibberish: {file_path.name}", log_callback)
        return
    ...
    if not text_embeddings:
        _log(f"⦸ Missing embeddings: {file_path.name}", log_callback)
        return
...
def perform_search(query_embedding: np.ndarray, queries: List[Any], sd: dict, models, collections, config, search_type: str, max_results: int) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        tokenized_search_term = normalize_text(sd['lexical_search_term'])
    ...
    for r in combined_results:
        if r['id'] == str(sd['attachment_path']):
...
def hybrid_search(queries: List[str], sd, models, collections, config, search_type: str) -> Optional[Dict[str, str]]:
    ...
    return perform_search(query_embedding, queries, sd, models, collections, config, search_type, config['max_results'])
...
Add progress indicator to sync"""