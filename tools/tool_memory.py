...
class Memory(BaseTool):
    ...
    agent_prompt = (
        ...
        "`list` everything, `create`, `update`, `delete`.\n"
        "The block above gives descriptions only, and a description is a map "
        "— `memory read` an entry before acting on it, and do not answer from "
        "a description alone. When something feels familiar mid-turn and was "
        "not listed, `memory list` or a `hybrid_search` scoped to the memory "
        "folder will find it — both are cheap, and usually cheaper than "
        "working it out again.\n"
        "Write when you learn an action worth repeating or avoiding — "
        ...
        "them down. One test: **will this change what an agent does next "
        "time?** If you cannot name the action, there is nothing to write. "
        "`list` first: if an entry already covers the situation, `update` it "
        "rather than adding a second, because two entries about one situation "
        "is how a corpus stops being useful.\n"
        "A **note** is one situation and its lesson. A **skill** is a "
        "repeatable procedure worth its own folder, for `references/` and "
        "`scripts/` you add with your file tools afterwards; reading one "
        "lists those files, and you open them with `read_file` when you reach "
        "the part that needs them.\n"
        "Nothing here needs approval, and nothing here can touch MEMORY.md."
    )
