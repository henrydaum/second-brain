...
class RunCommand(BaseTool):
    ...
    agent_prompt = (
        ...
        "**Anything expressible in Python should be a script, not a command.** A shell "
        "command is a process the kernel cannot see into, so it asks every time; a script "
        "is contained, so it asks nothing at all. Write it to the scripts directory and "
