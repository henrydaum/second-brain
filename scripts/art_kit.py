...
What's inside
    ...
    **Color** — ``hex_to_rgb``, ``rgb_to_hex``, ``mix_hex``, ``with_alpha``,
        ``oklch_to_rgb``, ``palette_color(t)`` (pre-bound to the active
        palette's luminance-sorted ramp)
    **Composition** — ``rule_of_thirds``, ``vogel_spiral``,
        ``jittered_grid``, ``regular_polygon``
    **Noise** — ``value_noise``, ``fbm``, ``value_noise_grid``,
        ``fbm_grid`` (scalar + vectorized pairs)
    **3D** — ``mesh``, ``cube_mesh``, ``render_3d`` (dependency-free
        painter's-algorithm renderer)
    **Flow** — ``flow_field`` (FBM-driven angle field)
