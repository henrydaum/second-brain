...
class MCPService(BaseService):
    ...
    def _oauth_provider(self, name: str, spec: dict):
        ...
        try:
            from .helpers.mcp_oauth import build_oauth_provider
        except Exception as e:
    ...
    def _oauth_prompt(self, name: str):
        ...
        def prompt(auth_url: str) -> str | None:
            ...
            req = runtime.request_input(
                session_key,
                f"Authorize MCP server '{name}'",
                (
                    f"{auth_url}\n\nOpen this link, sign in, and approve access. "
                    "You'll land on a page that won't load. Paste the full URL from "
                    "your browser's address bar, or just the code."
                ),
                type="string",
            )
            if not req.wait(timeout=float(self.config.get("mcp_oauth_timeout", 600) or 600)):
                req.metadata["timed_out"] = True
                runtime.answer_request(session_key, req.id, "")
                return None
            if req.metadata.get("cancelled"):
                return None
            return (req.value or "").strip() or None
        return prompt
