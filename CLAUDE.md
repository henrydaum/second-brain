...
`conv.create` **takes over a conversation nobody ever used** before inserting
another one (`ConversationRuntime.reuse_unused_conversation`, over
`Database.find_unused_conversation` + `claim_conversation`). Blank
conversations otherwise pile up forever: starting one is overwhelmingly
followed by starting another, and nothing reclaimed the leftovers —
`prune_expired` deletes by `updated_at`, and a state marker is written after
almost every action, so a conversation nobody ever spoke in never looks idle
enough to prune.

**Empty means no row that is not a marker.** A message *count* is never the
test: `save_state_marker` writes a `role='system'` row into
`conversation_messages`, so a conversation with nothing in it still has rows.
The predicate is stated as a negative rather than as the list of roles a
transcript uses, because `messages_to_history` silently drops any role it does
not recognise — a positive list would make an unfamiliar row invisible to a
reader and then delete it.

**Unused is stricter than empty**, and the extra clauses are the only intent an
empty conversation can carry: it must still hold a placeholder title
(`database.PLACEHOLDER_TITLES` — the defaults `create_conversation`,
`open_session`, the handler and `/new` each pass) and no category. That is what
keeps a `/clear`ed conversation, which is a deliberate decision to keep one
around empty, out of the candidate set without needing a case of its own.
`/new`'s hardcoded title is load-bearing and invisible: outside the set, reuse
never fires, which looks exactly like the feature being off, so
`tests/test_conversation_reuse.py` reads it back out of the command.

**The reuse decision is the handler's, and only the handler's.** Every other
caller of `create_conversation` — `open_session`, `ensure_conversation`, the
subagent supervisor — goes straight to an insert, so two children can never be
handed one conversation and `runtime/subagents.py` needs no opt-out to say so.

Three properties are load-bearing. **The caller's own live conversation is a
candidate only when it is being activated**: taking it over is safe because
activation rebuilds the session from the row (`load_conversation` always
constructs a fresh `RuntimeSession` from db rows), so wiping it underneath is a
reset — while a detached `activate=False` create handed the same row would
erase the state of a session somebody is sitting in and swallow their next
message. A `busy` session is exempt for the reason `compact_session` already
gives. **The claim re-checks the predicate inside its own `UPDATE`**, which is
what makes the whole thing safe to race: two claimants cannot both win, because
the loser's `WHERE` no longer matches. And **`REUSE_QUIET_SECONDS` is a
reservation with no state** — claiming bumps `updated_at`, hiding the row for a
window, because "held by a live session" is not true yet in the gap between a
create returning an id and its caller binding a session to it. The one case
where a delay would be felt, a person typing `/new` twice, never consults the
finder: their own conversation is addressed by id.

The claim resets `title`, `category`, `created_at` and
`last_title_check_message_count` with the messages. That last one is not
cosmetic: `list_conversations_for_title_check` compares the live count against
it, so a stale high-water mark hides the reused conversation from the
re-titling sweep. (`_conv_clear` has the same latent bug and is untouched.)
The ledger and the bus keep saying `conversation_create` / `created` and carry
`reused` alongside — a reuse *is* a conversation beginning, and anything asking
when one began should not have to learn two names.
