...
| max\_attachment\_size | If a text attachment is below this size, the entire document will be added to the context window of the LLM. If it is too large, it will only fit the most relevant chunks from the attachment (see setting below). Measured in tokens. Decrease if the context window is small. | 256-8192 | 1024 |
| n\_attachment\_chunks | Text chunks are extracted from attachments. These are used as additional queries for lexical and semantic search. | 1-10 | 3 |
