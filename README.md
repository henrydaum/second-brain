...
Second Brain can. Ask it for a tool it does not have. It writes the plugin, validates it against a capability contract, loads it into the running process, and uses it — without a restart, and without direct access to your disk, your network, or your kernel. Every effect it has on the outside world goes through a typed Request. The kernel classifies, approves, and records each one.

That is the whole idea: an agent that grows new abilities on demand, inside a boundary that does not grow with it.
...
Plugin code cannot touch the outside world. It cannot call `open()`. It cannot import `requests`. It asks — through one of 121 typed Requests — and the kernel decides.

A policy function classifies every Request by what it does, who asked, and where it is headed. Safe ones run. Unsafe ones surface a dialog with the full chain of provenance — written by the kernel, not by the plugin. Agent-authored code cannot lie about who called it because it does not get to write the dialog. Every piece of agent-written code runs in a subprocess with capped CPU, RAM, and filesystem visibility.

This is ~19,000 lines of working machinery with a green test suite, not a design document. Sandboxing the agent's own shell and filesystem access is a known-open problem across most self-hosted AI tools — usually an issue on the roadmap. Here it is the foundation everything else was built on top of.

### Self-evolution with a ceiling

The agent extends itself by writing plugins — tools, background tasks, services, slash commands, even whole frontends — and hot-loading them. But extending what it can ask for never extends what it is authorized to affect. The permission boundary is the kernel's, and self-written code is the least trusted code in the system.

Growth and containment usually trade off against each other. Here they are independent.

### A kernel small enough to actually trust

~17,000 lines, near-pure Python, boots fast. It holds a conversation, runs the state machine, persists to SQLite, and loads plugins. That is it. No built-in tools. No bundled model provider. Nothing you did not ask for.
...
> Conversations run through a turn-based state machine — think Magic: The Gathering: priority, legal actions, suspendable phases, serializable frames. Interrupted flows survive a crash. Frontends do not own conversation logic, so the REPL, Telegram, and the web app all get approvals, forms, cancellation, and history for free.
...
Or browse everything with `/packages` and pick a la carte — individual packages install by name, e.g. `/packages install tool_web_search`.
...
| **Build itself** | Author, validate, and hot-load new tools, tasks, services, commands, and frontends |
...
| `http_allowed_origins` | The origin you will serve the app from, or `*` while testing |
...
Your phone can now reach `http://100.101.102.103:8787` from anywhere, and nobody else can.
...
Serve `dist/` however you like. The simplest path is to set `http_static_dir` to that folder in step 2, so Second Brain itself serves the app on port 8787 and there is no CORS to configure.
...
It launches fullscreen, with no browser chrome. It is your assistant, running on your own hardware, in your pocket.
...
Ask for something it cannot do, and it builds it:
...
**Validation** is an AST pass that never imports the file. Checking code cannot run it. It catches direct effects and contract violations, and every error names the Request you should have used instead. It is a conformance linter, and the source says so out loud — static analysis of Python is defeatable by anyone trying, which is exactly why the subprocess exists.
...
**Foreign libraries are the honest limit.** A binary wheel's internal I/O cannot be turned into Requests. It is isolated and disclosed, not pretended away.
...
The package store is a git branch. Shipping a plugin is a pull request against [`store`](../../tree/store). Write it as a sandbox plugin, validate it, declare `dependencies_files` and `dependencies_pip`, and open the PR.
...
The value of AI is not any single feature. It is that AI can be built into almost anything. The role of the person is to guide it somewhere productive. So the kernel stays small, the contract stays strict, and what it becomes is up to you.
